...
function planReoptimize(
  ...
  // Step 1: locally repair operationally-protected rounds (active/published)
  // that still contain the outgoing seat — protected rows get narrow repair
  // only, never a rewrite.
  const protectedRepair = substitute
    ? planReplaceSwaps(state, outgoing, substitute, effectiveRound, (r) =>
        isOperationallyProtected(r, state.currentRound),
      )
    : planLocalRepair(state, outgoing, effectiveRound, (r) =>
        isOperationallyProtected(r, state.currentRound),
      );
  ...
  // Step 2: rebuild the reoptimizable rounds from scratch.
