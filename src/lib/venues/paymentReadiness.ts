...
export interface VenuePaymentSetup {
  ...
  venue: { id?: string; name: string; verification_approved_at: string | null; stripe_account_id?: string | null };
...
export function venuePaymentReadiness(data: VenuePaymentSetup) {
  ...
  const steps = [
    ...
    { id: 'owner', title: 'Verified venue owner', complete: !!data.venue.verification_approved_at, detail: 'Only the verified venue owner can connect the business’s Stripe account.' },
