  ...
  return (
    <div className="space-y-3">
      ...
      {matches.length === 0 ? (
        ...
        // Deterministic ordering: by day (asc), then court (asc, null last),
        // then scheduled_time, then id. Edits never re-shuffle the list.
        const nameOf = (id: string | null): string | null =>
          id ? (profilesById[id] ? resolvePlayerName(profilesById[id]) : null) : null;

        const inFilter = (m: LeagueMatch) => {
          if (statusFilter === "all") return true;
          if (statusFilter === "open") return m.status === "scheduled" || m.status === "in_progress";
          if (statusFilter === "needs_review") return m.status === "disputed" || m.status === "score_submitted";
          if (statusFilter === "done") return m.status === "verified" || m.status === "forfeit" || m.status === "canceled";
          return true;
        };
        const q = query.trim().toLowerCase();
        const inQuery = (m: LeagueMatch) => {
          if (!q) return true;
          const teamA = teams.find((t) => t.id === m.team_a_id)?.name ?? "";
          const teamB = teams.find((t) => t.id === m.team_b_id)?.name ?? "";
          const names = [
            teamA, teamB,
            nameOf(m.player_a_id), nameOf(m.player_b_id),
            nameOf(m.player_c_id), nameOf(m.player_d_id),
        ...
        const totals = {
          all: matches.length,
          open: matches.filter((m) => m.status === "scheduled" || m.status === "in_progress").length,
          needs_review: matches.filter((m) => m.status === "disputed" || m.status === "score_submitted").length,
          done: matches.filter((m) => m.status === "verified" || m.status === "forfeit" || m.status === "canceled").length,
        };
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <div className="flex items-center gap-1 flex-wrap">
                {([
                  ["all", "All", totals.all],
                  ["open", "Scheduled", totals.open],
                  ["needs_review", "Needs review", totals.needs_review],
              ...
              <div className="relative sm:ml-auto sm:w-64">
                ...
                <Input
                  ...
                  placeholder="Search team, player, court…"
            ...
            {visible.length === 0 ? (
              <EmptyState
                ...
                desc="Try clearing the search or picking a different chip."
              />
            ) : (
                ...
                {dayKeys.map((key) => {
                  ...
                  return (
                    <section key={key}>
                      <button
                        type="button"
                        onClick={() => setCollapsedDays((s) => ({ ...s, [key]: !collapsed }))}
                      ...
                      {!collapsed && (
                        <ul className="rounded-xl border border-border/70 bg-card overflow-hidden divide-y divide-border/60">
                          {list.map((m) => {
                            ...
                            const timeLabel = m.scheduled_time
                              ? new Date(m.scheduled_time).toLocaleTimeString(undefined, {
                                ...
                                >
                                  <div className="grid grid-cols-[56px_1fr_auto_1fr_auto] items-center gap-3 px-3 py-2.5">
                                    ...
                                    <div className="flex items-center gap-1.5 font-black tabular-nums px-1">
                                      {scoreShown ? (
                                        <>
                                          <span className={cn("text-lg leading-none", aWon ? "text-primary" : "text-muted-foreground")}>{m.team_a_score}</span>
