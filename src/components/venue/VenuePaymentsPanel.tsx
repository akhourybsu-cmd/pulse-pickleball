...
export function VenuePaymentsPanel({ venueId }: { venueId: string }) {
  ...
  const action = async (name: string, values: Record<string, unknown> = {}) => {
    ...
    try {
      ...
      if (!result || (name === 'save_venue' && result.saved !== true) || (name === 'resolve_cancellation' && result.resolved !== true)) throw new Error('The change was not confirmed. Refresh and check before trying again.');
      ...
      <section className="rounded-2xl border bg-card p-5 sm:p-6">
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-muted/40 p-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              variant="outline"
              disabled={!!busy || dirty || data.mode === "off" || data.transferred || !data.venue.verification_approved_at}
      ...
      <section className="rounded-2xl border bg-card p-5 sm:p-6" aria-busy={busy === 'save_venue'}>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          {data.courts.map((court: any) => (
            <div key={court.id} className="min-w-0 rounded-xl border p-4">
              ...
              {!court.is_active && <p className="mt-1 text-xs text-muted-foreground">Inactive · not bookable</p>}
        ...
        <div className="mt-5 flex items-start justify-between gap-4 rounded-xl border p-4">
          ...
          <Switch
            ...
            disabled={!!busy || (!accepting && (data.mode !== "live" || !connected || !data.venue.verification_approved_at))}
            className="shrink-0"
          />
        </div>
        <Button
