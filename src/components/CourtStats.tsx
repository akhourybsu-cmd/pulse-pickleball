...
export const CourtStats = ({ userId }: CourtStatsProps) => {
  ...
  return (
    <Card className="rounded-xl shadow-sm border border-gray-200 bg-gradient-to-b from-white to-gray-50 dark:from-card dark:to-card dark:border-border py-4">
      <CardHeader className="pb-2 border-b border-gray-100 dark:border-border mb-2">
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2 font-semibold text-[1.1rem] text-gray-800 dark:text-foreground">
              <MapPin className="w-5 h-5" />
              Stats by Court
            </CardTitle>
            <CardDescription className="text-sm text-gray-500 dark:text-muted-foreground mt-1">Your performance at different courts</CardDescription>
          </div>
          <Select value={selectedCourtId} onValueChange={setSelectedCourtId}>
            <SelectTrigger className="w-[250px] border-gray-300 dark:border-border rounded-md text-sm text-gray-600 dark:text-foreground">
      ...
      <CardContent className="pt-4">
        {!currentStats || currentStats.total_matches === 0 ? (
          ...
          <div className="grid grid-cols-4 divide-x divide-gray-100 dark:divide-border">
            <div className="flex flex-col items-center justify-center px-4">
              <p className="text-sm text-gray-500 dark:text-muted-foreground font-medium mb-2">Matches</p>
              <p className="text-2xl font-semibold text-gray-900 dark:text-foreground">{currentStats.total_matches}</p>
            </div>
            <div className="flex flex-col items-center justify-center px-4">
              <p className="text-sm text-gray-500 dark:text-muted-foreground font-medium mb-2">Record</p>
              <p className="text-2xl font-semibold text-gray-900 dark:text-foreground">{currentStats.wins}W - {currentStats.losses}L</p>
            </div>
            <div className="flex flex-col items-center justify-center px-4">
              <p className="text-sm text-gray-500 dark:text-muted-foreground font-medium mb-2">Win Rate</p>
            ...
            <div className="flex flex-col items-center justify-center px-4">
              <p className="text-sm text-gray-500 dark:text-muted-foreground font-medium mb-2">Avg Rating</p>
              <p className="text-2xl font-semibold text-gray-900 dark:text-foreground">{currentStats.avg_rating.toFixed(2)}</p>
            </div>
          </div>
        )}
      </CardContent>
