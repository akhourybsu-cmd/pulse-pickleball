...
export const HomepageNav = ({ isLoggedIn }: HomepageNavProps) => {
  ...
  const assessmentFirst = !isLoggedIn && isSkillAssessmentEnabled();
  const primaryHref = isLoggedIn ? "/player/dashboard" : assessmentFirst ? '/skill-assessment?source=nav' : SIGNUP_URL;
  const primaryLabel = isLoggedIn ? "Dashboard" : assessmentFirst ? 'Free assessment' : "Join PULSE";

  return (
    <header className="mkt-nav">
      <div className="mkt-container mkt-nav-inner">
        <Link to="/" className="mkt-logo-link" aria-label="PULSE home"><Logo compact className="mkt-logo" /></Link>
        ...
        <div className="mkt-nav-actions">
          ...
          <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
            ...
            <SheetContent className="mkt-mobile-menu">
              ...
              <SheetClose asChild><Link className="mkt-button mkt-button-gold" to={primaryHref}>{isLoggedIn ? "Open your dashboard" : assessmentFirst ? 'Take my free assessment' : "Create your free account"}<ArrowRight aria-hidden="true" /></Link></SheetClose>
