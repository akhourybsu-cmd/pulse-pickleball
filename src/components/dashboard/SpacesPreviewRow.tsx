...
export const SpacesPreviewRow = ({ userId, homeCourtId }: SpacesPreviewRowProps) => {
      ...
      <div className="space-y-2">
        ...
        <div className="bg-muted/20 rounded-xl p-3">
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            {[1, 2, 3].map((i) => (
              <div key={i} className="animate-pulse h-9 w-24 bg-background/80 rounded-full flex-shrink-0" />
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-2">
      <h3 className="text-xs font-medium text-muted-foreground px-1">Your Spaces</h3>
      {/* Single container with horizontal scroll chips */}
      <div className="bg-muted/20 rounded-xl p-3">
        <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-hide scroll-smooth">
          {spaces.map((space) => (
            <button
              key={space.id}
