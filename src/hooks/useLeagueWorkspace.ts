...
import type { LeagueMatch, LeagueMember, LeagueSession, LeagueTeam, LeagueTeamMember, LeagueSubstitute } from '@/lib/leagues/types';
...
export function useLeagueWorkspace(leagueId: string, seasonId: string, dataVersion: number, sections: Section[]) {
  ...
  const query = useQuery<WorkspaceData>({
    ...
    queryFn: async ({ signal }) => {
      ...
      const [matches, members, sessions, teams, subs] = await Promise.all([
      ...
      return {
        matches, members, sessions: sessions.sort((a,b) => (a.scheduled_date ?? '9999').localeCompare(b.scheduled_date ?? '9999')), teams, subs, batchByGroup,
