...
export default function TournamentsPublic() {
  ...
  const getTotalSpotsRemaining = async (event: TournamentEvent) => {
    if (!event.divisions) return null;
    
    let totalRemaining = 0;
    for (const division of event.divisions) {
      if (!division.max_teams) continue;
      ...
      const used = (teamCount || 0) + (regCount || 0);
      totalRemaining += Math.max(0, division.max_teams - used);
    }
                ...
                >
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      <Button 
                        className="flex-1"
                        onClick={() => navigate(`/auth?redirect=/tournament/${event.id}/register`)}
                      >
                        Register Team
                      </Button>
                      <Button 
