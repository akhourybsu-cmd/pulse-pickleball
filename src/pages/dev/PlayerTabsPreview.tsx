...
function MatchesPreview() {
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-3xl space-y-6 px-4 pb-10 pt-4 sm:px-6">
        ...
        <section><SectionHeader label="Recent matches" /><div className="space-y-3"><MatchRow /><MatchRow /></div></section>
      </main>
    </>
  );
}
    ...
    <>
      <SocialHero eyebrow="Groups" title="Community">
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <PlayerSegmentedControl value={view} onValueChange={setView} options={[{ value: "mine", label: "Mine", icon: Users, count: 3 }, { value: "explore", label: "Explore", icon: Compass, count: 8, accentCount: true }]} ariaLabel="Community views" layoutId="preview-community-view" className="min-w-[190px] flex-1" />
      ...
      <main className="mx-auto max-w-3xl space-y-3 px-4 pb-10 pt-4 sm:px-6">
  ...
  return (
    <>
      <SocialHero eyebrow="Player" title="Alex Morgan" action={<Button variant="outline" size="icon" className="h-10 w-10 rounded-xl bg-card/80" aria-label="Edit profile"><Edit3 className="h-[18px] w-[18px]" /></Button>}>
        ...
        <div className="mt-3 flex items-stretch gap-2.5"><Avatar className="h-[58px] w-[58px] shrink-0 rounded-2xl border-2 border-primary/30"><AvatarFallback className="rounded-2xl bg-primary/15 font-bold text-primary">AM</AvatarFallback></Avatar><div className="grid min-w-0 flex-1 grid-cols-3 gap-1.5"><SocialStatTile icon={Trophy} label="Rating" value="3.82" accent /><SocialStatTile icon={Swords} label="Matches" value="47" /><SocialStatTile icon={Trophy} label="Record" value="29–18" /></div></div>
      </SocialHero>
      <main className="mx-auto max-w-3xl space-y-6 px-4 pb-12 pt-4 sm:px-6">
        <button type="button" className="flex min-h-14 w-full items-center gap-3 rounded-2xl border border-border/60 bg-card/80 px-4 text-left shadow-[0_8px_28px_-22px_hsl(var(--foreground)/0.4)] active:scale-[0.99]"><Share2 className="h-5 w-5 text-primary" /><div><p className="text-sm font-semibold">Share your PULSE</p><p className="text-xs text-muted-foreground">Send your player profile</p></div><ChevronRight className="ml-auto h-4 w-4 text-muted-foreground/60" /></button>
...
export default function PlayerTabsPreview() {
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <header className="sticky top-0 z-40 border-b border-secondary-foreground/10 bg-secondary shadow-sm"><div className="mx-auto flex h-16 max-w-[1400px] items-center justify-between px-4"><Logo className="h-[52px] w-auto text-secondary-foreground" /><div className="flex gap-1 text-secondary-foreground"><Button variant="ghost" size="icon" className="h-9 w-9 rounded-full text-current hover:bg-secondary-foreground/10"><MessageCircle className="h-[18px] w-[18px]" /></Button><Button variant="ghost" size="icon" className="h-9 w-9 rounded-full text-current hover:bg-secondary-foreground/10"><Bell className="h-[18px] w-[18px]" /></Button><Avatar className="h-9 w-9 border border-secondary-foreground/20"><AvatarFallback className="bg-secondary-foreground/10 text-xs text-secondary-foreground">AM</AvatarFallback></Avatar></div></div></header>
