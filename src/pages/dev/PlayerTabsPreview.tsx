...
export default function PlayerTabsPreview() {
  ...
  return (
    <div className="min-h-screen bg-background pb-20 lg:pb-0">
      <header className="sticky top-0 z-40 border-b border-secondary-foreground/10 bg-secondary shadow-sm"><div className="mx-auto flex h-[68px] max-w-[1400px] items-center justify-between gap-2 px-4 sm:h-[72px] sm:px-6 lg:grid lg:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] lg:gap-5 lg:px-8"><Logo className="h-[46px] w-auto shrink-0 text-secondary-foreground sm:h-[58px] lg:h-[60px] lg:justify-self-start" /><nav className="hidden items-center justify-center lg:flex lg:justify-self-center"><div className="flex items-center gap-1 rounded-2xl border border-secondary-foreground/10 bg-secondary-foreground/[0.045] p-1">{navItems.map(({value,label,icon:Icon})=>{const selected=value===active;return <button key={value} type="button" onClick={()=>value!=="home"&&setActive(value)} className={cn("flex h-10 items-center gap-2 rounded-xl px-3 text-sm font-medium xl:px-4",selected?"bg-background/95 text-foreground shadow-sm":"text-secondary-foreground/70")}><Icon className={cn("hidden h-[17px] w-[17px] xl:block",selected&&"text-primary")} />{label}</button>})}</div></nav><div className="flex shrink-0 gap-1 text-secondary-foreground sm:gap-2 lg:justify-self-end"><Button variant="ghost" size="icon" className="h-9 w-9 rounded-full text-current hover:bg-secondary-foreground/10"><MessageCircle className="h-[18px] w-[18px]" /></Button><Button variant="ghost" size="icon" className="h-9 w-9 rounded-full text-current hover:bg-secondary-foreground/10"><Bell className="h-[18px] w-[18px]" /></Button><Avatar className="h-9 w-9 border border-secondary-foreground/20"><AvatarFallback className="bg-secondary-foreground/10 text-xs text-secondary-foreground">AM</AvatarFallback></Avatar></div></div></header>
