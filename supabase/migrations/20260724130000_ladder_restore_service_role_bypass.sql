-- =====================================================================
-- Restore the service_role bypass on the ladder process/generate RPCs.
--
-- 20260719190000 added a `v_is_service := auth.role() = 'service_role'`
-- bypass to ladder_finalize_batch and ladder_generate_batch so the
-- ladder-advance edge function (which runs with the service role, no user
-- JWT) could process/generate batches for auto-advance.
--
-- Two later migrations re-applied these functions WITHOUT the bypass and
-- granted EXECUTE only to `authenticated`:
--   • 20260723160559 → ladder_finalize_batch
--   • 20260723185719 → ladder_generate_batch
-- Under the service role auth.uid() is NULL, so every ladder-advance call
-- now raises 'Authentication required' — auto-advance is completely dead.
--
-- This migration re-applies the CURRENT bodies of both functions verbatim
-- and re-adds the bypass + service_role grant. The bypass is safe: only a
-- service-role JWT carries auth.role()='service_role', a claim a normal
-- user's token can never hold, so authenticated users still hit the
-- is_league_admin check.
-- =====================================================================

-- ---- ladder_finalize_batch (PROCESS-ONLY) + service bypass ----------
...
-- ---- ladder_generate_batch (generic) + service bypass ---------------
