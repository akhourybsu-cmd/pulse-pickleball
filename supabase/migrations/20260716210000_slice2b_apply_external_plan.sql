...
--
-- The Slice 3 planner (`scoreRemainingSchedule`) runs in the rr-manage-
-- participant Edge Function, which snapshots the event, computes the plan
-- (including full `reoptimize` round rewrites the SQL planner cannot do), and
-- passes it here as `p_plan` + `p_plan_hash`.
--
-- Division of authority (Amendment 4): the planner decides what is PREFERABLE;
-- this RPC still decides what is SAFE to commit — it re-locks the event,
-- re-checks auth + schedule_version + idempotency, validates the plan, and
-- applies it atomically, bumping schedule_version exactly once.
--
-- Backward compatible: when `p_plan` is NULL the RPC behaves exactly as the
-- Slice 2a hardening/completion version (internal local-repair planner), so
-- existing direct callers (manageParticipantRpc.ts remove/replace) are
-- unaffected. The idempotency input-hash formula is intentionally UNCHANGED so
-- pre-migration completed requests still replay byte-for-byte.
--
-- Only two behavioural additions:
--   1. p_plan present  -> trust it (validate ok/action) instead of recomputing.
--   2. a new `rewrite_round` plan op            -> regenerate one round.
-- The existing `swap_identity` apply loop is reused verbatim; the Edge Function
-- emits swap_identity ops in the same DB shape (from_/to_ profile/guest ids).
-- =========================================================================
...
BEGIN
  ...
  -- Idempotency hash: UNCHANGED formula (excludes p_plan/p_plan_hash) so that
  -- requests recorded by the Slice 2a function still replay correctly and so
  -- that the deterministic plan (a pure function of these same inputs) does not
  ...
  -- ---- Plan resolution -------------------------------------------------
  -- External plan (Slice 2b/3): trust the Edge Function's plan after
  -- validating it is well-formed and matches the requested action. Otherwise
  -- fall back to the Slice 2a internal local-repair planner.
  IF p_plan IS NOT NULL THEN
    v_plan_source := 'external';
    IF coalesce((p_plan ->> 'ok')::boolean, false) IS NOT TRUE THEN
      RAISE EXCEPTION 'invalid_plan' USING ERRCODE = '22023',
        DETAIL = jsonb_build_object('code','invalid_plan','retryable',false,
          'reason','plan_not_ok')::text;
    END IF;
    IF (p_plan ->> 'action') IS DISTINCT FROM p_action THEN
      RAISE EXCEPTION 'plan_action_mismatch' USING ERRCODE = '22023',
        DETAIL = jsonb_build_object('code','plan_action_mismatch','retryable',false,
          'plan_action', p_plan ->> 'action', 'request_action', p_action)::text;
    END IF;
    v_plan_result := p_plan;
  ELSIF p_regen_mode = 'reoptimize' THEN
  ...
  IF v_plan_ops IS NOT NULL THEN
    ...
    LOOP
      ...
      ELSIF (v_op ->> 'op') = 'rewrite_round' THEN
        -- Full reoptimize of one reoptimizable round. Only unplayed, unlocked,
        -- non-voided, non-superseded rows are removed and rebuilt; rows with
        -- any score history are never touched (invariant #2).
        v_round := (v_op ->> 'round_no')::integer;
        ...
        FOR v_m IN SELECT * FROM jsonb_array_elements(v_op -> 'matches')
        LOOP
          INSERT INTO public.round_robin_schedule (
            event_id, round_no, court_no, is_bye,
            a1_player_id, a2_player_id, b1_player_id, b2_player_id,
            ...
            p_event_id, v_round, (v_m ->> 'court_no')::integer,
            coalesce((v_m ->> 'is_bye')::boolean, false),
            nullif(v_m ->> 'a1_player_id','')::uuid, nullif(v_m ->> 'a2_player_id','')::uuid,
            nullif(v_m ->> 'b1_player_id','')::uuid, nullif(v_m ->> 'b2_player_id','')::uuid,
            nullif(v_m ->> 'a1_guest_id','')::uuid,  nullif(v_m ->> 'a2_guest_id','')::uuid,
            nullif(v_m ->> 'b1_guest_id','')::uuid,  nullif(v_m ->> 'b2_guest_id','')::uuid
          );
        END LOOP;

        INSERT INTO public.round_robin_audit(event_id, editor_id, change_type, changes, reason)
        VALUES (p_event_id, v_actor, 'schedule_rewrite_round',
          jsonb_build_object(
            'request_id', p_request_id,
            'round_no', v_round,
            'matches', v_op -> 'matches'), p_reason);
      END IF;
    END LOOP;
  END IF;
...
REVOKE ALL ON FUNCTION public.rr_manage_participant(
  uuid, uuid, uuid, text, text, integer, text, boolean, jsonb, jsonb, jsonb, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.rr_manage_participant(
  uuid, uuid, uuid, text, text, integer, text, boolean, jsonb, jsonb, jsonb, text) TO authenticated;
