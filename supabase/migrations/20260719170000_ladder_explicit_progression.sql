-- =====================================================================
-- Ladder: decouple PROCESS from GENERATE-NEXT (explicit progression)
--
-- Previously ladder_finalize_batch both processed a batch AND generated
-- the next one in the same call. The sequential-ladder spec requires
-- generation to be an EXPLICIT organizer step ("Process Batch 1" →
-- "Generate Batch 2" → … → "Generate Week 2"), and the next WEEK must
-- never be auto-created. So:
--
--   • ladder_finalize_batch is now PROCESS-ONLY: it validates, writes the
--     result snapshot + movements, marks the batch finalized (processed),
--     and stops. It never generates a downstream stage.
--   • ladder_generate_batch(season, start_snapshot, plan) is a new generic
--     generator: it creates ONE batch (groups + games) from an EXISTING
--     processed snapshot. Used by both "generate next batch" (same week)
--     and "generate next week", each an explicit organizer action.
--
-- Both remain transactional + idempotent (unique keys, row-safe).
-- =====================================================================

-- ---- PROCESS-ONLY finalize ------------------------------------------
...
BEGIN
  ...
  -- Whole-batch completeness across EVERY group (server-side).
  ...
  -- Same player set as the batch start (integrity).
...
-- ---- generic generator: ONE batch from an existing snapshot ---------
...
BEGIN
  ...
  THEN
    ...
    VALUES (
      ...
      jsonb_build_object('week', (v_fb ->> 'week')::int, 'batch', (v_fb ->> 'batch')::int,
...
COMMENT ON FUNCTION public.ladder_finalize_batch IS
  'PROCESS-ONLY: validates completeness + integrity, writes the result '
  'snapshot + movements, marks the batch finalized. Never generates a '
  'downstream stage — generation is an explicit organizer step.';
COMMENT ON FUNCTION public.ladder_generate_batch IS
  'Generates ONE ladder batch (groups + games) from an existing processed '
  'snapshot. Idempotent. Used by the explicit generate-next-batch and '
  'generate-next-week organizer actions.';
