...
--
--  #1  ladder_reopen_batch: when discarding downstream batches it cascade-
--      deletes their league_matches, orphaning the bridged `matches` rows as
--      still-approved/rated (the rating bridge only fires on INSERT/UPDATE).
--      Now it deletes those linked `matches` rows and replays ratings so the
--      discarded games' rating effect is backed out.
--  #2  cancel_ladder_sub_request: refuse once the week is generated (every
--      sibling has this guard; without it a canceled sit-out desyncs the
--      already-built groups from finalize's live sit-out read).
--  #4  swap_league_week_player: reject a fill-in already playing another
--      group of the same batch/week (double-booking).
--  #3  swap_league_week_player: add a service_role bypass so the auto-advance
--      function can re-seed assigned subs into same-week batches 2+.
--  #6  submit_league_match_score: reject a tied score (would otherwise write
--      a "stuck" verified game that blocks progression).
-- =====================================================================

-- ---- #1: reopen backs out discarded downstream ratings --------------
...
-- ---- #2: cancel refuses once the week is generated ------------------
...
BEGIN
  ...
  END IF;
  -- Roster is frozen into the groups once the week is generated; canceling a
  -- resolved sit-out here would desync finalize's live sit-out read.
  IF EXISTS (
...
-- ---- #3 + #4: swap gets a service bypass + double-booking guard -----
...
BEGIN
  ...
  -- #4: the fill-in can't already be scheduled in ANOTHER game of the same
  -- scope (batch if given, else the season's upcoming games) — that would
  -- double-book them onto two courts at once.
  IF EXISTS (
  ...
  -- Audit actor: the caller, or (service-role auto-advance) the league owner.
