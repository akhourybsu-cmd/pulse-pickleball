...
COMMENT ON FUNCTION public.recalculate_all_ratings IS
  'Full sequential rating replay. Placement branch is gated by rating_parameters.placement_enabled (default false = current ELO exactly).';
COMMENT ON FUNCTION public.placement_forces_full_recalc IS
  'True when placement is enabled and the match involves a still-placing player, so the incremental path must delegate to recalculate_all_ratings().';