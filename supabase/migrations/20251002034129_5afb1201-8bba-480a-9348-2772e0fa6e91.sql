-- Fix ambiguous column reference by renaming the rating_change variable
...
DECLARE
  ...
  p3_rating NUMERIC;
  p4_rating NUMERIC;
  calculated_rating_change NUMERIC;  -- Renamed to avoid ambiguity
...
BEGIN
  ...
  -- Reset all players' current_rating to their week_start_rating
  ...
  LOOP
    ...
    calculated_rating_change := calculate_pulse_rating_change(
    ...
    SET 
      ...
      rating_after = p1_week_start + calculated_rating_change,
      rating_change = calculated_rating_change
    WHERE match_id = match_record.match_id AND player_id = match_record.player_ids[1];
    
    UPDATE match_participants
    SET 
      rating_before = p2_week_start,
      rating_after = p2_week_start + calculated_rating_change,
      rating_change = calculated_rating_change
    WHERE match_id = match_record.match_id AND player_id = match_record.player_ids[2];
    
    UPDATE match_participants
    SET 
      rating_before = p3_week_start,
      rating_after = p3_week_start - calculated_rating_change,
      rating_change = -calculated_rating_change
    WHERE match_id = match_record.match_id AND player_id = match_record.player_ids[3];
    
    UPDATE match_participants
    SET 
      rating_before = p4_week_start,
      rating_after = p4_week_start - calculated_rating_change,
      rating_change = -calculated_rating_change
    WHERE match_id = match_record.match_id AND player_id = match_record.player_ids[4];
    
    -- Update current_rating (live estimate, not official score)
    UPDATE profiles SET current_rating = p1_rating + calculated_rating_change WHERE id = match_record.player_ids[1];
    UPDATE profiles SET current_rating = p2_rating + calculated_rating_change WHERE id = match_record.player_ids[2];
    UPDATE profiles SET current_rating = p3_rating - calculated_rating_change WHERE id = match_record.player_ids[3];
    UPDATE profiles SET current_rating = p4_rating - calculated_rating_change WHERE id = match_record.player_ids[4];
  END LOOP;
  
  -- Recalculate all player stats (total matches from ALL history, not just current week)
