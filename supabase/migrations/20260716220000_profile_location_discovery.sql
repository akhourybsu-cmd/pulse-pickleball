-- =========================================================================
-- Friend discovery by location (opt-in, reciprocal).
--
-- Adds a verified home location + coordinates to profiles and a
-- distance-ranked discovery RPC. This intentionally introduces a NEW trust
-- model on top of the existing network-scoped connect flow: a player is only
-- ever surfaced by location when THEY have opted in
-- (discoverable_by_location = true), and the RPC only returns results to a
-- caller who has ALSO opted in and set a location — so seeing nearby players
-- and being seen are the same switch (reciprocity).
--
-- Raw coordinates are never exposed through profiles_public; discovery goes
-- through this SECURITY DEFINER RPC, which returns only a coarse city label +
-- rounded distance.
-- =========================================================================
...
-- Partial index for the discovery scan — only opted-in rows with coordinates.
...
-- -------------------------------------------------------------------------
-- discover_players_nearby: distance-ranked, opt-in, reciprocal.
-- Returns nothing unless the caller has opted in AND set coordinates.
-- Excludes self and anyone already in a friendship row (pending / accepted /
-- blocked, either direction). Distance via haversine (km).
-- -------------------------------------------------------------------------
...
BEGIN
  ...
  -- Reciprocity gate: you can only browse nearby players if you are yourself
  -- discoverable and have a location set.
