-- ============================================================================
-- PULSE Skill Assessment — Organizer Visibility
--
-- Lets authorized league organizers see useful placement info WITHOUT ever
-- exposing raw survey responses, question-by-question answers, contradiction
-- details, or private scoring internals. Authorization is enforced at the
-- data layer (SECURITY DEFINER + can_view_player_skill / is_league_admin),
-- not by hiding UI.
--
-- Adds:
--   • skill_organizer_reviews — org-scoped, auditable, NON-destructive notes.
--   • get_player_skill_card()  — one player's SANITIZED organizer card.
--   • get_league_skill_cards() — compact list (adds review_recommended).
--   • record_skill_review()    — record an organizer review (never changes
--                                the player's global self-assessed level).
--
-- Does NOT touch the PULSE Performance Rating, scoring, the question bank,
-- or the player-facing tables' shapes.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- Organizer reviews — organization/league scoped, append-only audit trail.
-- ----------------------------------------------------------------------------
...
-- Only league admins (for that league) or system admins may read reviews.
-- Players are NOT admins of leagues they merely play in, so a player can
-- never read organizer notes about themselves.
...
-- Inserts must be by the acting organizer for a league they admin.
...
-- Append-only for organizers; system admins retain full control.
...
-- ----------------------------------------------------------------------------
-- record_skill_review — non-destructive, org-scoped, auditable. Never mutates
-- the player's global self-assessed level or any profile value.
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Audit trail (org-scoped). Note text is intentionally NOT logged here.
...
-- ----------------------------------------------------------------------------
-- get_player_skill_card — one player's SANITIZED organizer card.
-- Strips contradictions + internal meta from the snapshot; exposes only
-- placement-useful fields. Authorized via can_view_player_skill (owner /
-- public / league-admin-of-a-shared-league, respecting the visibility flag).
-- latest_review_status is returned only to organizers, never to the player.
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  RETURN QUERY
    SELECT
      ...
      -- SANITIZED: remove contradiction details + internal meta before exposure.
...
-- ----------------------------------------------------------------------------
-- get_league_skill_cards — compact list for organizers, now with a
-- review_recommended flag. Return-type change ⇒ drop + recreate.
-- ----------------------------------------------------------------------------
