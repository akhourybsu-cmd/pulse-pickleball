-- =====================================================================
-- Audit fixes for player sub-requests.
--
--  #1  request_ladder_sub: reject a week whose date is already in the past.
--  #2  request_ladder_sub: reject Week 1 (its roster is the initial ladder;
--      sit-outs/subs only apply from week 2, so a Week 1 request could only
--      be resolved as sub/declined and would confuse the organizer).
--  #4  resolve_ladder_sub_request: a fill-in can't be assigned to two
--      players the same week, and can't be someone already on the ladder.
--  #10 set_ladder_week_sitout: manually bringing a player back in reopens
--      any request that had been resolved as a sit-out, so it can't go stale.
-- =====================================================================
...
BEGIN
  ...
  END IF;
  -- #2: Week 1 is the starting roster.
  IF v_week < 2 THEN
    RAISE EXCEPTION 'Week 1 is set by the initial ladder — you can''t request a sub for it'
      USING ERRCODE = '22023';
  END IF;
  -- #1: no requesting a week that has already come and gone.
...
-- ---- #4: fill-in uniqueness on resolution ---------------------------
  ...
  IF p_resolution = 'sub' THEN
    ...
    IF NOT v_ok THEN
      RAISE EXCEPTION 'The fill-in must be an active substitute or member of this season'
        USING ERRCODE = '22023';
    END IF;
    -- #4a: not already covering another player the same week.
    IF EXISTS (
      ...
      RAISE EXCEPTION 'That fill-in is already covering another player this week'
        USING ERRCODE = '22023';
    END IF;
    -- #4b: not someone already scheduled to play (on the current ladder).
...
-- ---- #10: un-sitting reopens a sit-out-resolved request -------------
...
BEGIN
  ...
  ELSE
    ...
    -- #10: a request that was resolved as a sit-out is no longer settled —
    -- reopen it so it can't linger as "sitting out" while the player is back
    -- in the roster (and so generation re-gates until it's decided again).
    UPDATE public.ladder_sub_requests
