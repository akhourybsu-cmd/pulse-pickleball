...
-- =========================================================================
-- Slice 2a hardening: preview is read-only, effective_round is server-derived,
-- search_path fully locked down, plan types are semantically accurate,
-- identity uniqueness enforced on replace.
-- =========================================================================

-- Drop previous signatures (both had trailing p_effective_round).
DROP FUNCTION IF EXISTS public.rr_manage_participant(uuid, uuid, uuid, text, text, integer, text, boolean, jsonb, jsonb, integer);
DROP FUNCTION IF EXISTS public.rr_plan_participant_change(uuid, uuid, text, uuid, integer);

-- -------------------------------------------------------------------------
-- Planner (internal). No caller-controlled effective_round; derived here.
-- search_path = pg_catalog only. All references fully qualified.
-- Returns { ok, plan_type, plan[], fairness_triggers[] } or { ok:false, code }.
-- -------------------------------------------------------------------------
CREATE FUNCTION public.rr_plan_participant_change(
  p_event_id        uuid,
...
DECLARE
  ...
  v_bye_row       record;
BEGIN
  SELECT COALESCE(current_round, 1) INTO v_effective_round
  ...
  IF p_action = 'replace' THEN
    ...
    FOR r IN
      SELECT id, round_no, court_no
       ...
       WHERE event_id = p_event_id
         AND round_no >= v_effective_round
         AND locked_at IS NULL AND voided_at IS NULL
         AND superseded_by_schedule_id IS NULL AND is_bye = false
         AND (
    ...
    v_plan_type := CASE WHEN jsonb_array_length(v_plan) = 0
                        THEN 'no_schedule_change'
                        ELSE 'replace_identity' END;
    RETURN jsonb_build_object('ok', true, 'plan_type', v_plan_type,
      'effective_round', v_effective_round,
      'plan', v_plan, 'fairness_triggers', to_jsonb(v_triggers));
  END IF;
    ...
    LOOP
      ...
      IF v_bye_row.participant_id IS NULL THEN
        ...
        RETURN jsonb_build_object(
          ...
          'retryable', false, 'effective_round', v_effective_round,
  ...
  IF p_action = 'restore' THEN
    RETURN jsonb_build_object('ok', true, 'plan_type', 'restore_identity',
      'effective_round', v_effective_round,
      'plan', '[]'::jsonb, 'fairness_triggers', to_jsonb(v_triggers));
  END IF;
...
REVOKE ALL ON FUNCTION public.rr_plan_participant_change(uuid, uuid, text, uuid) FROM PUBLIC, anon, authenticated;

-- -------------------------------------------------------------------------
-- Public RPC. No p_effective_round. Preview is read-only.
-- search_path = pg_catalog only; every table, function, extension qualified.
-- -------------------------------------------------------------------------
...
BEGIN
  ...
  -- Idempotency ledger is checked for ALL requests, but only WRITTEN for non-preview.
  ...
  -- Lock event only for non-preview. Preview uses a plain read.
  ...
  IF p_action = 'restore' THEN
    IF v_participant.status = 'active' THEN
      RAISE EXCEPTION 'invalid_state_transition' USING ERRCODE = '22023',
        DETAIL = jsonb_build_object('code','invalid_state_transition','retryable',false)::text;
    END IF;
  ...
  -- Substitute validation (identity-level uniqueness).
  IF p_action = 'replace' THEN
    ...
    -- Identity-level dedup: the incoming registered user must not already have
    -- any participant row in this event (any status).
    IF v_sub_profile IS NOT NULL AND EXISTS (
      SELECT 1 FROM public.round_robin_players
       WHERE event_id = p_event_id
         AND player_id = v_sub_profile
  ...
  -- Detect active matches for the outgoing participant.
  ...
  -- Planner: reoptimize/auto-fail returns reoptimization_required in 2a.
  ...
  v_effective_round := public.coalesce((v_plan_result ->> 'effective_round')::integer,
                                       v_event.current_round, 1);
  ...
  VALUES
    (p_event_id, v_actor, 'participant_' || p_action, p_player_id, p_reason,
...
-- Retire the obsolete preview_completed status. No preview writes exist anymore.
UPDATE public.rr_participant_mutation_requests
   SET status = 'completed'
 WHERE status = 'preview_completed';
