-- Fix the notify_home_court_comment function to not reference non-existent type field
...
BEGIN
  ...
  SELECT cp.court_id, cp.user_id, cp.session_date, cp.max_players
  INTO v_post
  FROM court_posts cp
  ...
  IF v_post.session_date IS NOT NULL AND v_post.max_players IS NOT NULL THEN
    v_post_type := 'LFG';
  ELSE
    v_post_type := 'post';
  END IF;
  ...
  SELECT 
    p.id,
    'court_activity',
    v_commenter_name || ' commented on a ' || v_post_type,
