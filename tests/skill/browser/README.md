...
Diagram clarity checks (September 23): all 64 scene options render, stay within the phone-width card, and have a caption. Gold arrows show the shot or the moving player; white dashed arrows show incoming shots or partner movement. Deep serves show the back third of the diagonal service box. The serving-team return is labelled as the opponent’s shot. The high-drop transition scenario stays still and has no Play control. Playback uses SVG coordinates without video embeds: verify the ball/player changes position, Pause retains its exact coordinates, Resume continues, Replay restarts after completion, and choosing a new scene resets playback. Reduced-motion rendering has static arrows and no playback controls (also checked in the UI tests).

Use **Stall request** to simulate an account request that never resolves, even after abort. **Report load** followed by reload must show Retry after 15 seconds; restore None and Retry to recover. **Answer save** must retain the selection and re-enable Save after 15 seconds. **Activity metadata** must not delay a successfully saved answer. **Result save** must leave the finalizing screen after 25 seconds, reload the durable answers and allow another completion attempt. Restore None to finish, then reload to verify the saved report. These four recovery paths were exercised against the real assessment hook and local adapter.
