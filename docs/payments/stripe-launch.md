...
**Corrective backend deployed; web/PWA publication and hosted verification in progress. Live launch remains blocked.** The September 10 failed-refund and stale-owner-queue findings have a tested corrective implementation and complete migration `20260921120000_refund_failure_recovery.sql`. Backend commit `9d1a817a89e762f1afaddd5a5acfcb136dd16702` passed Supabase deployment run `34539452226`, including migration application and Edge Functions. See the [failure evidence and corrective implementation](../reports/2026-09-10-palace-payment-sandbox.md#corrective-implementation--prepared-locally-not-deployed). Do not treat deployment success alone as proof that the hosted test ledger has been repaired. Keep live payments disabled.
...
Follow-up progress: insufficient-funds decline, failed and successful 3DS, and leaving/resuming the same unpaid checkout passed through the hosted app. All 115 focused payment/private-venue tests passed. This does not cover later failed refunds: the hosted failure fixture exposed the blocker above. A scheduled pass checked an actual pending order without error, but missed-payment fulfillment recovery is still unproven. Remaining cases below must not be marked complete on the strength of unit tests alone.
