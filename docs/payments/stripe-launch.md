...
**Corrective release published and the original failed-refund fixture verified on the hosted app. Live launch remains blocked by the remaining acceptance matrix.** Backend commit `9d1a817a89e762f1afaddd5a5acfcb136dd16702` passed Supabase run `34539452226`, applying `20260921120000_refund_failure_recovery.sql` before deploying the Edge Functions. Web/PWA commit `08498530680ed4b53a86f3cbbcedf46e77239d25` passed Firebase run `34539979268`, independently reproducing 1,132 passing tests. The five-minute recovery automatically corrected the existing test record to $0 refunded / refund failed and reopened owner review, without recreating the canceled reservation or issuing another refund. Both hosted views and the owner status-check action were verified. See the [publication evidence](../reports/2026-09-10-palace-payment-sandbox.md#corrective-publication--september-10-2026). Keep live payments disabled.

Status: backend migration and Edge Functions deployed successfully in commit `3655049222e3d5f2f2d18613821888e705877de2` (Supabase run `34468994469`), then web/PWA commit `e3b18171ec7b480b426c9674ec54800b0d5ff106` deployed successfully (Firebase run `34469652725`). With subsequent explicit approval, test-mode secrets, test OAuth/redirect, both event destinations and the five-minute recovery scheduler are configured. Platform signed-event delivery, duplicate replay and recovery HTTP checks passed; see the [sandbox setup evidence](../reports/2026-09-10-stripe-sandbox-setup.md). The approved [Palace follow-up](../reports/2026-09-10-palace-payment-sandbox.md) verifies the $10/month test subscription and cancellation, completed test-account onboarding, a $10 rental paid automatically, player cancellation request, owner full refund, signed rental/refund events, receipt and platform-versus-venue portal/customer separation. Palace remains private and live-billing-disabled; no real money moved. The remaining two-venue, failure/3DS/expiry, refund-edge-case, renewal/recovery and installed PWA acceptance checks below are still required before live launch.
...
Check `cron.job_run_details` and the corresponding `net._http_response` records. A green cron invocation only means the HTTP request was queued; verify HTTP 200 and `failed: 0` in the response. Monitor failures, oldest pending purchases, pending/failed refunds and Stripe event delivery failures. The corrective recovery pass handles up to 100 pending checkouts and rotates through up to 100 settled payment orders per run, including already-refunded orders; it no longer limits refund recovery to open requests. Refund reads run in batches of five, and their attempted-check timestamps advance before Stripe calls so an unavailable account does not monopolize the next batch. At larger volume, recovery latency depends on queue size; monitor the oldest unchecked payment. Invoice renewals and subscription status still rely on signed webhook delivery; use Stripe event replay for missed renewals rather than assuming the checkout scheduler backfills all invoices.
...
## Required sandbox acceptance (partially completed; live launch blocked)

Follow-up progress: insufficient-funds decline, failed and successful 3DS, and leaving/resuming the same unpaid checkout passed through the hosted app. The original later-failed-refund fixture now passes corrective hosted checks: scheduled recovery repaired its amount and reopened owner review, and both published views reflect the failure. All 1,132 tests passed in release CI. A scheduled pass also checked an actual pending order without error, but missed-payment fulfillment recovery is still unproven. Remaining cases below must not be marked complete on the strength of unit tests alone.
