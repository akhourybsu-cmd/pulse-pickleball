...
- Explicit user approval: publish all pending. Backend commit `9d1a817a89e762f1afaddd5a5acfcb136dd16702` was pushed to main first. Supabase run `34539452226` succeeded, including migration runner checks, pending migration application and Edge Function deployment.
- Web/PWA publication and hosted verification are in progress. The local implementation evidence above remains a historical pre-deployment checkpoint, not a claim that the hosted fixture was already repaired at that time.
- No live-payment activation or Android Studio / Google Play deployment is authorized by this release.
